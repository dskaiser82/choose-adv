...
export async function ensureSchema() {
  ...
  const campaignRecordsExists = await tableExists("campaign_records");
  if (!campaignRecordsExists) return;

  const campaignRowsPayload = await tursoPipeline([
    {
      type: "execute",
      stmt: {
        sql: `select record_type, slug, title, content_json, created_at, updated_at from campaign_records order by updated_at asc`,
      },
    },
  ]);

  for (const row of rows(campaignRowsPayload, 0)) {
    const recordType = rowValue(row, 0) ?? "";
    const slug = rowValue(row, 1) ?? crypto.randomUUID();
    const title = rowValue(row, 2) ?? "Untitled";
    const contentRaw = rowValue(row, 3) ?? "{}";
    const createdAt = rowValue(row, 4) ?? new Date().toISOString();
    const updatedAt = rowValue(row, 5) ?? createdAt;
    ...
    if (recordType === "world") {
      await upsertWorld({
        id: "world-main",
        name: String(content.name ?? title),
        tone: typeof content.tone === "string" ? content.tone : null,
        technologyLevel: typeof content.technologyLevel === "string" ? content.technologyLevel : null,
        summary: String(content.summary ?? ""),
        majorPowers: Array.isArray(content.majorPowers) ? content.majorPowers.filter((x): x is string => typeof x === "string") : [],
        regions: Array.isArray(content.regions) ? content.regions.filter((x): x is string => typeof x === "string") : [],
        locations: Array.isArray(content.locations) ? content.locations.filter((x): x is string => typeof x === "string") : [],
        updatedAt,
      });
    }

    if (recordType === "character") {
      await upsertCharacter({
        id: "character-main",
        name: String(content.name ?? title),
        status: String(content.status ?? "alive"),
        kingdom: typeof content.kingdom === "string" ? content.kingdom : null,
        region: typeof content.region === "string" ? content.region : null,
        formerAffiliation: typeof content.formerAffiliation === "string" ? content.formerAffiliation : null,
        role: typeof content.role === "string" ? content.role : null,
        yearsOfService: typeof content.yearsOfService === "number" ? content.yearsOfService : null,
        specializations: Array.isArray(content.specializations)
          ? content.specializations.filter((x): x is string => typeof x === "string")
          : [],
        updatedAt,
      });
    }
    ...
    if (recordType === "scene") {
      await upsertScene({
        ...
        narration: String(content.narration ?? ""),
        suggestedChoices: Array.isArray(content.suggestedChoices)
          ? content.suggestedChoices.filter((x): x is string => typeof x === "string")
          : [],
        actionDraft: typeof content.actionDraft === "string" ? content.actionDraft : null,
        updatedAt,
      });
    }
    ...
    if (recordType === "event") {
      await appendEvent({
        ...
        eventKey: slug,
        title,
        summary: String(content.summary ?? content.narration ?? ""),
        action: typeof content.action === "string" ? content.action : null,
        createdAt,
      });
    }
  }
